    ...
    for v in vpcs:
        summaries.append(
            VpcSummary(
                ...
                name=name_from_tags(v.get("Tags", [])),
    ...
    for s in subnets:
        summaries.append(
            SubnetSummary(
                ...
                name=name_from_tags(s.get("Tags", [])),
    ...
    for g in groups:
        summaries.append(
            SecurityGroupSummary(
                ...
                vpc_id=optional_str(g.get("VpcId")),
    ...
    for lb in lbs:
        ...
        summaries.append(
            AlbSummary(
                ...
                dns_name=optional_str(lb.get("DNSName")),
                scheme=optional_str(lb.get("Scheme")),
                state=optional_str(state.get("Code")) if isinstance(state, dict) else None,
                vpc_id=optional_str(lb.get("VpcId")),
                type=optional_str(lb.get("Type")),
                arn=optional_str(lb.get("LoadBalancerArn")),
            )
        )
    return summaries
    ...
    for tg in tgs:
        ...
        summaries.append(
            TargetGroupSummary(
                ...
                protocol=optional_str(tg.get("Protocol")),
                port=int(port) if port is not None else None,
                vpc_id=optional_str(tg.get("VpcId")),
                target_type=optional_str(tg.get("TargetType")),
                health_check_path=optional_str(tg.get("HealthCheckPath")),
                arn=optional_str(tg.get("TargetGroupArn")),
            )
        )
    return summaries

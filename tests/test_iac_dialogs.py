...
def test_cdk_dialog_command_building(tk_root: tk.Tk) -> None:
    ...
    dialog.select_subcommand(_CDK_SUBCOMMANDS[0])
    dialog.field_vars["--long"].set(True)
    cmd = dialog.build_command()
    assert "list" in cmd
    assert "--long" in cmd

    # Test 'deploy' with STACKS
    # deploy is index 3
    dialog.select_subcommand(_CDK_SUBCOMMANDS[3])
    dialog.field_vars["STACKS"].set("stack1 stack2")
    cmd = dialog.build_command()
    ...
    cmd_dry = dialog.build_command(dry_run=True)

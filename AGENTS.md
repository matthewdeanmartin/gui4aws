...
- Use `boto3-stubs` type stubs for mypy coverage of AWS API calls.
- Never hard-code credentials or region strings. Rely on `~/.aws/config`, environment variables,
  or instance metadata. Use `boto3.Session` to make the credential source explicit in tests.
- Mock AWS calls in unit tests with `moto` or `pytest-mock` rather than hitting real endpoints.
